...
def build_model(args):
    ...
    elif args.architecture == 'RefineGNNplus_attonly':
        return rev_plus.RevisionDecoder_plus(args).cuda()
    else:
...
if __name__ == '__main__':
    ...
    args = get_args() # get argument variables
    set_SEED(args) # set seed for reproducibility
    os.makedirs(args.save_dir, exist_ok=True) # create folder to save models
    ...
    train_dict['model'].append( args.architecture)
    train_dict['best_epoch'].append( best_epoch)
    train_dict['test_ppl'].append(test_ppl)
    train_dict['test_rmsd'].append(test_rmsd)

    results_df = pd.DataFrame(train_dict)
